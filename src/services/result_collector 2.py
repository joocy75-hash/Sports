...
class ResultCollector:
    ...
    async def collect_round_results(
        ...
        # 복수 베팅 적중 조합 계산
        # 복수 베팅 경기들의 적중 여부를 기반으로 조합 수 계산
        multi_games = [r for r in game_results if r.predicted_multi]
        if multi_games:
            # 모든 복수 베팅 경기가 적중한 경우의 조합 수 계산
            # 각 경기가 multi_correct이면 해당 경기는 1조합, 아니면 0조합
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if not response_data or not isinstance(response_data, dict):
                    logger.error(f"유효하지 않은 API 응답 구조: {data}")
                    return []
    ...
    def _parse_result(self, match_end_val: str, game_type: str, api_result: Dict) -> str:
        ...
        - 원정팀 6점 이상 승 → "패"
        """
        if not match_end_val:
            return ""

        val = match_end_val.strip().lower()
