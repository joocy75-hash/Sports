...
# 이변 확률 계산 공식
upset_probability = (
    (signal_1 * 0.35) +  # 확률 분포 애매함 (1위-2위 차이 < 15%)
    (signal_2 * 0.30) +  # AI 모델 간 불일치 (표준편차 > 20%)
    (signal_3 * 0.20) +  # 폼-예측 상충 (최근 폼과 예측 반대)
    (signal_4 * 0.15) +  # 랭킹 불일치 (강팀인데 낮은 승률)
    (1 - model_consensus) * 20
)
...
3. **14경기 중 일부만 분석** - 반드시 14경기 전체 분석 필요
...
2. 5개 AI 앙상블로 합의된 결과 도출
3. 이변 감지 로직으로 불확실한 경기 탐지
4. 14경기 전체 마킹 리스트 + 복수 4경기
5. 텔레그램으로 자동 알림
